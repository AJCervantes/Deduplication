  ...
  for (i in 1:(rows - 1)){
    ...
    for (j in (i:rows)){
      ...
      if (percent > 0){
        ...
        #Name check
        if(percent >= (cutoff - 50) && name_first[i] != "sr" && name_first[j] != "sr"){
      ...
      if (percent >= cutoff){
        case1 <- case_id[i]
        case2 <- case_id[j]
        exl2 <- which(exclusion_2 == case2)
        exl1 <- which(exclusion_1 == case1)
        if(length(exl1) != 0 && length(exl2) != 0 &&  exl1[1] == exl2[1]){
          next
        }
        ...
        cat(case1, append = TRUE)
        cat(",", append = TRUE)
        cat(case2, append = TRUE)
        cat("\n\n", append = TRUE) 
        is_match[i] <- TRUE
        is_match[j] <- TRUE
        break
